...
function parseDecimalAR(v) {
  ...
  // si ya es numero, listo
  if (typeof v === "number" && Number.isFinite(v)) return v;

  let s = String(v);

  // NBSP (formato contabilidad suele meterlo)
  s = s.replace(/\u00A0/g, " ").trim();
  ...
  // quitar moneda/letras y dejar solo dígitos, separadores y signo
  // "$ 3.400,00" -> "3.400,00"
  ...
  if (hasComma && hasDot) {
    ...
    // 3400 o 3.400 (si vino así, lo tratamos como miles)
    s = s.replace(/\s+/g, "");
    // si tiene solo ".", asumimos miles: 3.400 -> 3400
...
export async function runImportServiclub({ filePath, originalName }) {
  const schema = env.import.schema; // importacionxls
  const tableName = "Reporte General Serviclub";
  ...
  if (!insertExcelCols.length) {
    ...
    return {
      ...
      insertedRows: 0,
      skippedDuplicates: 0,
      note: "No matchearon columnas del excel con la tabla Serviclub."
    };
  }
